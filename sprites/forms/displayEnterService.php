<?php 
function displayEnterService($message,$errorArray,$valuesArray,$errorFlag,$errorType){

if(($valuesArray["serviceID"] =="")){	
$query = "SELECT serviceID FROM services ORDER BY serviceID DESC LIMIT 0,1";
$result = sql_query($query, $message);
if(mysql_num_rows($result) >= 1) {
	$errorFlag=true;			
	$row = mysql_fetch_array($result);
	$valuesArray["serviceID"]= htmlspecialchars(stripslashes($row["serviceID"])) + 1;
}

if(mysql_num_rows($result) == 0) {
	$errorFlag=true;			
	$row = mysql_fetch_array($result);
	$valuesArray["serviceID"]= 1000;
}
    ...
    <td><table id="forms" cellpadding="0" cellspacing="0" border="0" >
        <tr>
          ...
          <td align="center" ><table cellpadding="0" cellspacing="0" border="0" align="right" id="rightBoby">
              <tr>
                <td valign="top"><form action="/Myshop/administrator.php" method="post" name="enterService" id="enterService" >
                    <input id="frmName" name="frmName" value="enterService" type="hidden" />
